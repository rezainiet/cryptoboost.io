...
const NonInvestedUsers = () => {
    ...
    const fetchNonInvestedUsers = async (page = 1, search = "", limit = usersPerPage) => {
        setLoading(true)
        try {
            const response = await fetch(
                `https://cryptoboost-io.onrender.com/api/admin/users/non-invested?page=${page}&limit=${limit}&search=${search}`,
            )
    ...
    const handleExport = async () => {
        ...
        try {
            const filename = `non-invested-users-page-${currentPage}-${new Date().toISOString().split("T")[0]}.csv`
    ...
    useEffect(() => {
        fetchNonInvestedUsers(currentPage, searchTerm, usersPerPage)
        ...
        <div className="p-6 bg-gray-100 dark:bg-gray-900 min-h-screen text-gray-900 dark:text-gray-100">
            <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex gap-3">
                    ...
                    <button
                        onClick={() => fetchNonInvestedUsers(currentPage, searchTerm, usersPerPage)}
                        disabled={loading}
            ...
            <div className="flex gap-4 mb-4">
                <input
                    ...
                    className="flex-1 px-4 py-2 border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-transparent"
                />

                <select
                    value={usersPerPage}
                    onChange={(e) => handleItemsPerPageChange(Number(e.target.value))}
                    className="px-4 py-2 border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-transparent"
                >
